...
class UserRepository {
    ...
    static find(id: number) {
        return new Promise((resolve, reject) => {
            const querry = `
                ...
                WHERE u.id=${id};
            `;
            MySQL.query(querry, (error: Error, result: FindResult[]) => {
    ...
    static getIdByToken(token: string) {
        return new Promise((resolve, reject) => {
            ...
            `;
            MySQL.query(getIdUser, (error: Error, result: TokenResult[]) => {
                if (error) return reject(error);
                return resolve(result);
            });
        })
    ...
    static list() {
        return new Promise((resolve, reject) => {
            const listUsers = `
                ...
                INNER JOIN permission p ON p.id = u.id_permission
            `;
            MySQL.query(listUsers, (error: Error, result: FindResult[]) => {
                if (error) return reject(error);
                return resolve(result);
            });
        });
    ...
    static checkPermission(id: number, type: number) {
        return new Promise((resolve, reject) => {
            ...
            `;
            MySQL.query(checkPermission, ((error: Error, result: PermissionResult[]) => {
                if (error) return reject(error);
